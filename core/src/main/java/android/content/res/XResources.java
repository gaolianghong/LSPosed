...
import static de.robv.android.ghlosed.XghledHelpers.decrementMethodDepth;
import static de.robv.android.ghlosed.XghledHelpers.findAndHookMethod;
import static de.robv.android.ghlosed.XghledHelpers.getBooleanField;
import static de.robv.android.ghlosed.XghledHelpers.getLongField;
import static de.robv.android.ghlosed.XghledHelpers.getObjectField;
import static de.robv.android.ghlosed.XghledHelpers.incrementMethodDepth;
...
import de.robv.android.ghlosed.IXghledHookZygoteInit;
import de.robv.android.ghlosed.XC_MethodHook;
import de.robv.android.ghlosed.XC_MethodHook.MethodHookParam;
import de.robv.android.ghlosed.XghledBridge;
import de.robv.android.ghlosed.XghledBridge.CopyOnWriteSortedSet;
import de.robv.android.ghlosed.XghledInit;
import de.robv.android.ghlosed.callbacks.XC_LayoutInflated;
import de.robv.android.ghlosed.callbacks.XC_LayoutInflated.LayoutInflatedParam;
import de.robv.android.ghlosed.callbacks.XCallback;
import ghlosed.dummy.XResourcesSuperClass;
import ghlosed.dummy.XTypedArraySuperClass;
...
//			Log.w(XghledBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	 ...
	 * {@link IXghledHookZygoteInit#initZygote initZygote()} for all apps. As there is no
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { XghledBridge.log(t); }
				} else if (replacement instanceof Integer) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						XghledBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					XghledBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	public void getValue(int id, TypedValue outValue, boolean resolveRefs) throws NotFoundException {
		...
		if (replacement instanceof XResForwarder) {
			...
			if (replacement != null) {
				XghledBridge.log("Replacement of resource ID #0x" + Integer.toHexString(id) + " escaped because of deprecated replacement. Please use XResForwarder instead.");
			}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				XghledBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			XghledBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
