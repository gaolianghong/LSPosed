...
import de.robv.android.ghlosed.XC_MethodHook;
import de.robv.android.ghlosed.XC_MethodReplacement;
import de.robv.android.ghlosed.XghledBridge;
import de.robv.android.ghlosed.XghledHelpers;
...
public class ParasiticManagerHooker {
    ...
    private static void sendBinderToManager(final ClassLoader classLoader, IBinder binder) {
        try {
            var clazz = XghledHelpers.findClass("org.lsposed.manager.Constants", classLoader);
            var ok = (boolean) XghledHelpers.callStaticMethod(clazz, "setBinder",
    ...
    private static void hookForManager(ILSPManagerService managerService) {
        var managerApkHooker = new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ...
                ApplicationInfo appInfo = (ApplicationInfo) XghledHelpers.getObjectField(bindData, "appInfo");
                XghledHelpers.setObjectField(bindData, "appInfo", getManagerPkgInfo(appInfo).applicationInfo);
            }
        };
        XghledHelpers.findAndHookMethod(ActivityThread.class,
                "handleBindApplication",
                "android.app.ActivityThread$AppBindData",
                managerApkHooker);

        var unhooks = new XC_MethodHook.Unhook[]{null};
        unhooks[0] = XghledHelpers.findAndHookMethod(
                LoadedApk.class, "getClassLoader", new XC_MethodHook() {
                    @Override
                    protected void afterHookedMethod(MethodHookParam param) {
                        var pkgInfo = getManagerPkgInfo(null);
                        if (pkgInfo != null && XghledHelpers.getObjectField(param.thisObject, "mApplicationInfo") == pkgInfo.applicationInfo) {
        ...
        var activityClientRecordClass = XghledHelpers.findClass("android.app.ActivityThread$ActivityClientRecord", ActivityThread.class.getClassLoader());
            ...
            protected void afterHookedMethod(MethodHookParam param) {
                for (var i = 0; i < param.args.length && activityClientRecordClass.isInstance(param.thisObject); ++i) {
                    if (param.args[i] instanceof ActivityInfo) {
                        ...
                        Hookers.logD("loading state of " + aInfo.name);
                        states.computeIfPresent(aInfo.name, (k, v) -> {
                            XghledHelpers.setObjectField(param.thisObject, "state", v);
                            return v;
                        });
                        persistentStates.computeIfPresent(aInfo.name, (k, v) -> {
                            XghledHelpers.setObjectField(param.thisObject, "persistentState", v);
                            return v;
                        });
                    }
                }
        ...
        XghledBridge.hookAllConstructors(activityClientRecordClass, activityHooker);

        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1) {
            XghledBridge.hookAllMethods(XghledHelpers.findClass("android.app.ActivityThread$ApplicationThread", ActivityThread.class.getClassLoader()), "scheduleLaunchActivity", activityHooker);
        }

        XghledBridge.hookAllMethods(ActivityThread.class, "handleReceiver", new XC_MethodReplacement() {
            @Override
        ...
        XghledBridge.hookAllMethods(ActivityThread.class, "installProvider", new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ...
                if (ctx != null && info != null && pkgInfo != null) {
                    ...
                    if (originalContext == null) {
                        ...
                        XghledHelpers.setObjectField(originalPkgInfo, "mPackageName", packageName);
                        originalContext = (Context) XghledHelpers.callStaticMethod(XghledHelpers.findClass("android.app.ContextImpl", null),
        ...
        XghledHelpers.findAndHookMethod(ActivityThread.class, "deliverNewIntents", activityClientRecordClass, List.class, new XC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam param) {
        ...
        XghledHelpers.findAndHookMethod(WebViewFactory.class, "getProvider", new XC_MethodReplacement() {
            @Override
            protected Object replaceHookedMethod(MethodHookParam param) {
                var sProviderInstance = XghledHelpers.getStaticObjectField(WebViewFactory.class, "sProviderInstance");
                if (sProviderInstance != null) return sProviderInstance;
                //noinspection unchecked
                var providerClass = (Class<WebViewFactoryProvider>) XghledHelpers.callStaticMethod(WebViewFactory.class, "getProviderClass");
                ...
                try {
                    ...
                    XghledHelpers.setStaticObjectField(WebViewFactory.class, "sProviderInstance", sProviderInstance);
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                try {
                    ...
                    if (record instanceof IBinder) {
                        record = ((ArrayMap<?, ?>) XghledHelpers.getObjectField(param.thisObject, "mActivities")).get(record);
                        if (record == null) return;
                    }
                    XghledHelpers.callMethod(param.thisObject, Build.VERSION.SDK_INT >= Build.VERSION_CODES.P ? "callActivityOnSaveInstanceState" : "callCallActivityOnSaveInstanceState", record);
                    var state = (Bundle) XghledHelpers.getObjectField(record, "state");
                    var persistentState = (PersistableBundle) XghledHelpers.getObjectField(record, "persistentState");
                    var aInfo = (ActivityInfo) XghledHelpers.getObjectField(record, "activityInfo");
        ...
        XghledBridge.hookAllMethods(ActivityThread.class, "performStopActivityInner", stateHooker);
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1)
            XghledHelpers.findAndHookMethod(ActivityThread.class, "performDestroyActivity", IBinder.class, boolean.class, int.class, boolean.class, stateHooker);
    }
